...
describe('FaqPage', () => {
  ...
  it('renders the faq hero, ctas and CMS faq content', async () => {
    ...
    expect(screen.getByRole('link', { name: /ask for a quote/i })).toHaveAttribute('href', '/custom-cakes')
    ...
    expect(screen.getByRole('link', { name: /custom quote form/i })).toHaveAttribute('href', '/custom-cakes')
  ...
  it('renders a graceful empty state and omits FAQPage structured data when there are no FAQs', async () => {
    ...
    expect(screen.getAllByRole('link', { name: /ask for a quote/i })[0]).toHaveAttribute('href', '/custom-cakes')
    expect(blocks).toHaveLength(1)
