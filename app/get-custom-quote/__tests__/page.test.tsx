...
describe('GetCustomQuotePage', () => {
  ...
  it('renders the refreshed quote landing sections in order', async () => {
    ...
    expect(
      screen.getAllByRole('listitem').slice(0, 3).map((item) => item.textContent)
    ).toEqual([
      '✓Handmade in Leeds',
    ...
    expect(screen.getByText('Most enquiries receive a reply within 24 hours.')).toBeInTheDocument()
